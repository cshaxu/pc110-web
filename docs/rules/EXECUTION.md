...
- Governance and architecture documentation changes `docs/` by default. Product
  code changes one approved boundary at a time: `src/app/`, `src/emulator/`,
  `qemu/`, `scripts/`, `tests/`, or the intentionally versioned public runtime
  and media directories.
- **Commit-scope rule (non-negotiable):** every authored non-merge commit has
  one declared boundary: governance (`docs/` and directly related root
  metadata), application, emulator runtime, QEMU build, tests, or release
  package. A feature spanning boundaries is a stack of separately reviewable
  commits.
